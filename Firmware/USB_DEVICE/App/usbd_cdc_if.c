...
uint8_t SLCAN_StrIndex = 0;
SLCAN_StatusTypeDef status;

uint32_t CAN_TX_MAILBOX_USED;
uint8_t data[8];
		  ...
		  if(status == SLCAN_OK){ // Fails if string is not parsed correctly
				  ...
				  SLCAN_CommandStringTx[0] == 'R') 		// Check if Transmit (t,T,r,R) Command
			  {
				  if (HAL_CAN_AddTxMessage(&hcan1, &header, data, &CAN_TX_MAILBOX_USED) == HAL_OK)
					  ...
					  LED_RedOn(); 			// TODO Add a proper handler for failed transmission
				  	  HAL_Delay(75); 	  	// Consider putting Tx function in "slcan.c"
				  	  LED_RedOn()
			  }
		  }
		  else {
